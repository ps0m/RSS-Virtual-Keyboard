...
const VirtualKeyboard = {
  ...
  init() {
    ...
    for (let i = 0; i < 64; i += 1) {
      const key = new MyKey(
        ...
        ListSetKey[i].code,
        ListSetKey[i].textEn,
        ListSetKey[i].textEnShift,
        ListSetKey[i].textRu,
        ListSetKey[i].textRuShift,
        ListSetKey[i].size,
        ListSetKey[i].assignment,
      );
      ...
      this.button.dataset.code = this.keySet[i].code;
  ...
  setValue() {
    ...
    for (let i = 0; i < this.keySet.length; i += 1) {
      if (!this.properties.isShift && !this.properties.isCapsLock) {
        this.mainKeyboard.children[i].textContent = this.keySet[i][`text${language}`];
      }
      if (this.keySet[i].assignment !== "functional") {
        if (!this.properties.isShift && this.properties.isCapsLock) {
          this.mainKeyboard.children[i].textContent = this.keySet[i][`text${language}`].toUpperCase();
        }
        if (this.properties.isShift && !this.properties.isCapsLock) {
          this.mainKeyboard.children[i].textContent = this.keySet[i][`text${language}Shift`].toUpperCase();
        }
        if (this.properties.isShift && this.properties.isCapsLock) {
          this.mainKeyboard.children[i].textContent = this.keySet[i][`text${language}Shift`].toLowerCase();
        }
      }
    }
  },
  ...
  setEventListener() {
    ...
    //     this.properties.isCapsLock = true;
    //     this.setValue();
    ...
    this.mainKeyboard.addEventListener("mousedown", (event) => {
      ...
      if (this.keySet[number].assignment !== "functional") {
        this.mainField.value += this.mainKeyboard.children[number].textContent;
      }
      if (this.keySet[number].code === "Enter") {
        this.mainField.value += "\n";
      }
      if (this.keySet[number].code === "Tab") {
        event.preventDefault();
        this.mainField.value += "    ";
      }
      if (this.keySet[number].code === "CapsLock") {
        this.changeActiveCapsLock();
        this.setValue();
      }
      if (this.keySet[number].code === "ShiftLeft" || this.keySet[number].code === "ShiftRight") {
        if (event.repeat) {
          return;
        }
        this.properties.isShift = !this.properties.isShift;
  ...
  changeActiveCapsLock() {
    const capsLock = document.querySelector(".key__caps");
    capsLock.classList.toggle("key__caps_active");
...
// VirtualKeyboard.setMainFieldFocus();
